...
gloso = read.csv(file.path(DataDir,"ABCD_lavaan_gfactor_loso.csv"))
gloso$eventname = "baseline_year_1_arm_1"
names(gloso)[1:4] = c("G_lavaan","S1_lavaan","S2_lavaan","S3_lavaan")
gloso = gloso[,-which(names(gloso)=="site_id_l")]

nih = read.abcd(file.path(ABCDDataDir,"abcd_tbss01.txt"))
nih = nih[,c("subjectkey","eventname","nihtbx_picvocab_uncorrected",
...
data = multi.merge(fam,site,motion,pdem,gloso,nih,ps,lmt,by=mergecols)
